...
func (d Decimal) A() Decimal {
	ret := d
	ret.exp -= ret.exp - 1
	ret.exp -= int32(len(d.value.String()))
	return ret
}
...
func (d Decimal) Log(base int) Decimal {
	return (d.Ln()).Div(NewFromInt(int64(base)).Ln())
}

// Common Bases are hardcoded with precision of 2^11 digits
var ln10Decimal, _ = NewFromString("2.")
var decimalZero = NewFromInt(0)
var decimalOne = NewFromInt(1)
var decimalTwo = NewFromInt(2)
var decimalTen = NewFromInt(10)

// Log10 returns the log base 10 of d
func (d Decimal) Ln() Decimal {
	if d.Equal(decimalTen) {
		ret := ln10Decimal
		return ret
	}

	A := d.A()
	n := int32(len(d.value.String())) + d.exp
	fmt.Printf("x = %sE%d-1\n", A, n)

	y := (A.Sub(decimalOne)).Div(A.Add(decimalOne))
	...
	for i := 0; i < 1000; i++ {
		last = sum
		sum = sum.Add(num.Div(NewFromInt(int64(2*i + 1))))
		num = num.Mul(y).Mul(y)
		if last.Equal(sum) {
			break
		}
	...
	return (decimalTwo.Mul(sum)).Add(NewFromInt32(n - 1).Mul(ln10Decimal))
}
